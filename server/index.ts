...
app.delete('/api/organizations/:id', async (c) => {
  ...
  //  - status !== 'Inativo' (se foi inativada, esteve em uso — Regra 5);
  //  - nunca teve contrato, de qualquer status (Regra 4.1);
  //  - zero contas vinculadas, de qualquer status (Regra 4.2 / bottom-up).
  ...
  if (orgContracts.length > 0) {
    return c.json({ error: 'ja-teve-contrato' }, 422)
  }
...
app.delete('/api/solutions/:id', async (c) => {
  ...
  // Exclusão física — reunião 03/09/2026. Componente NÃO bloqueia (não é filho
  // da hierarquia). Contrato de qualquer status trava para sempre.
...
/**
 * GET /api/accounts/:id/usuario-grupos
 * Retorna todas as tuplas (userId, grupoId, grupoNome, grupoEscopo) de membros de
 * grupos visíveis nesta conta (grupos da conta + grupos org-scoped herdados), em
 * uma única query — evita N+1 ao exibir a coluna "Grupo" na listagem de usuários.
 */
app.get('/api/accounts/:id/usuario-grupos', async (c) => {
  const accountId = c.req.param('id')

  const gruposVisiveis = await db
    .select()
    .from(grupos)
    .where(or(eq(grupos.accountId, accountId), isNull(grupos.accountId)))

  if (gruposVisiveis.length === 0) return c.json([])

  const grupoIds = gruposVisiveis.map((g: any) => g.id)
  const links = await db
    .select()
    .from(usuarioGrupos)
    .where(inArray(usuarioGrupos.grupoId, grupoIds))
 ...
 * Remove também atribuições (DocNix) e component_permissions (FGA) do membro.
 */
app.delete('/api/instancias/:id/membros/:membroId', async (c) => {
  const instanciaId = c.req.param('id')
  const membroId    = c.req.param('membroId')

  // Buscar o membro para obter entidadeTipo/entidadeId (precisamos para limpar component_permissions)

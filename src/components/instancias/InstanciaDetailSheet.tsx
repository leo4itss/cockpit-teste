            ...
            {canManage && isDocNix && fasesLoaded && slotsLoaded && (!mostraFases || !mostraPerfil) && (
              <div className="border-b border-gray-100 px-6 py-4 space-y-3">
                ...
                {!mostraFases && (
                  !showAtivarFases ? (
                    <button
                      onClick={() => setShowAtivarFases(true)}
                    ...
                    <div className="border border-dashed border-blue-200 rounded-xl p-4 space-y-3 bg-blue-50/40">
                      <p className="text-xs font-medium text-gray-700">Nome da primeira fase</p>
                      <div className="flex gap-2">
                        <input
                          ...
                          onKeyDown={async e => {
                            if (e.key === 'Enter' && novaFaseNome.trim()) {
                              const nova = await api.createFase(instancia!.id, { nome: novaFaseNome, ordem: 0 })
                              setFases([nova])
                              setFasesLoaded(true)
                              setNovaFaseNome('')
                              setShowAtivarFases(false)
                          ...
                          className="flex-1 text-sm border border-gray-300 rounded-md px-2 py-1.5 outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                          autoFocus
                        />
                        <button
                          onClick={async () => {
                        ...
                        <button
                          onClick={() => { setShowAtivarFases(false); setNovaFaseNome('') }}
                ...
                {/* Ativar Perfil de Objeto */}
                {!mostraPerfil && (
                  !showAtivarPerfil ? (
                    <button
                      onClick={() => setShowAtivarPerfil(true)}
                      className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 font-medium"
                    >
                      <span className="w-5 h-5 rounded-full border-2 border-blue-600 flex items-center justify-center text-xs leading-none">+</span>
                      Ativar Perfil de Objeto
                    ...
                    <div className="border border-dashed border-blue-200 rounded-xl p-4 space-y-3 bg-blue-50/40">
                      <p className="text-xs font-medium text-gray-700">Nome do primeiro slot de perfil</p>
                      <input
                        type="text"
                        placeholder="Ex: Aprovadores"
                        value={novoSlotNome}
                        onChange={e => setNovoSlotNome(e.target.value)}
                        className="w-full text-sm border border-gray-300 rounded-md px-2 py-1.5 outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                        autoFocus
                      />
                      <select
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => { setShowAtivarPerfil(false); setNovoSlotNome(''); setNovoSlotAtribId('') }}

...
/**
 * Devolve o conjunto de IDs dos descendentes (filhos, netos, …) de `grupoId`
 * dentro do array `grupos`. Inclui o próprio `grupoId` para facilitar checagens.
 * Proteção contra ciclos via Set de visitados.
 */
function getDescendentes(grupoId: string, grupos: Grupo[]): Set<string> {
  const result = new Set<string>([grupoId])
  let frontier = [grupoId]
  while (frontier.length > 0) {
    const next: string[] = []
    for (const g of grupos) {
      if (g.parentId && frontier.includes(g.parentId) && !result.has(g.id)) {
        result.add(g.id)
        next.push(g.id)
      }
    }
...
// Editor inline de grupo pai — select com confirm/cancel.
interface ParentEditorProps {
  currentParentId: string | null | undefined
  grupo:           Grupo
  grupos:          Grupo[]
  onSave:          (novoParentId: string | null) => Promise<void>
}
function ParentEditor({ currentParentId, grupo, grupos, onSave }: ParentEditorProps) {
  const [editing, setEditing] = useState(false)
  const [draft, setDraft]     = useState<string>(currentParentId ?? '')
  const [saving, setSaving]   = useState(false)

  // Opções válidas: mesmo escopo, exclui o próprio e descendentes (evita ciclo).
  const opcoes = useMemo(() => {
    const descendentes = getDescendentes(grupo.id, grupos)
    return grupos.filter(g =>
      g.escopo === grupo.escopo &&
      g.status === 'Ativo' &&
      !descendentes.has(g.id)
    )
  }, [grupo.id, grupo.escopo, grupos])

  const nomeAtual = currentParentId
    ? (grupos.find(g => g.id === currentParentId)?.nome ?? currentParentId)
    : null
  ...
  if (!editing) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-medium text-[#374151]">{nomeAtual ?? '(nenhum)'}</span>
        <button
          type="button"
          onClick={() => { setDraft(currentParentId ?? ''); setEditing(true) }}
          className="text-[#9ca3af] hover:text-blue-600 transition-colors"
          title="Editar grupo pai"
        >
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <select
        ...
        disabled={saving}
        className="text-xs px-2 py-1 border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
      >

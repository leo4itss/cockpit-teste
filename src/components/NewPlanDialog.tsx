...
export function NewPlanDialog({ open, onClose, onSave, initialPlan, tiposLicenca }: Props) {
  ...
  function handleLicensingChange(
    index: number,
    field: keyof Licensing,
    ...
    setLicensings(ls => ls.map((l, i) => {
      ...
      // Atualiza campos denormalizados quando o tipo muda
      ...
      // Recalcula preço mensal automaticamente
  ...
  function handleSave() {
    if (!name.trim()) return
    if (licensings.length === 0) return  // ao menos um tipo de licença obrigatório
              ...
              <div className="flex flex-col gap-6">
                {licensings.map((lic, i) => (
                  <div key={i} className="flex flex-col gap-4">
                    ...
                    {/* Tipo de Licença select + remover */}

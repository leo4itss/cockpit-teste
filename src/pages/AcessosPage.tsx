...
export function AcessosPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(u => {
      ...
      return matchSearch && matchGrupo && matchArea && matchPapel && matchStatus
    })
  }, [users, searchUsers, filtroGrupo, filtroArea, filtroPapel, filtroStatus, userGruposMap])

  // Áreas distintas presentes nos usuários carregados (para o dropdown de filtro)
  const areasDisponiveis = useMemo(
    () => [...new Set(users.map(u => u.area).filter(Boolean))].sort(),
    [users]
  )
